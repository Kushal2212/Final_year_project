...
# evaluate_ensemble
#
# KEY FIX: both models now receive raw [0, 255] pixels.
# The Rescaling layer is baked INSIDE each saved .keras model, so applying
# any external rescaling (/ 255 or / 127.5 - 1) causes double-rescaling
# and destroys MobileNetV2 accuracy (100% → 27%).
# ─────────────────────────────────────────────────────────────────────────
def evaluate_ensemble(test_dir):
    import tensorflow as tf
    from tensorflow.keras.models import load_model
